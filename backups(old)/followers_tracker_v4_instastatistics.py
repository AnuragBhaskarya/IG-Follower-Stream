#!/usr/bin/env python3
"""
Instagram Follower Tracker v4 - Using InstaStatistics API
Fastest yet - 2-second cache refresh, CORS allows any origin!
"""
...
CHECK_INTERVAL = 1        # minimal delay - API cache is 2 seconds anyway
...
# ---------------------------
# API Function (Super Fast - 2s cache!)
# ---------------------------
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                logger.warning(f"Could not find follower count in response")
            else:
...
def play_gain_audio(diff: int) -> None:
    ...
    # Determine category based on follower gain
    if diff <= 10:
        category = diff
    elif diff <= 100:
...
# ---------------------------
def main() -> None:
    logger.info("🚀 Starting Instagram Follower Tracker v4 (InstaStatistics API - 2s cache!)")
    wait_for_internet()
