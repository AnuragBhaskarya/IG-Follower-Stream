#!/usr/bin/env python3
"""
Instagram Follower Tracker v2 - Using Blastup API (CSRF token required)
More complex than v3/v4, but works as a backup option.
"""
...
def fetch_follower_count() -> Optional[int]:
    """Fetches the follower count using the API."""
    try:
...
def send_notification(message: str) -> None:
    try:
        subprocess.run(["notify-send", "Instagram Update", message], check=True)
    except Exception as e:
...
# ---------------------------
def main() -> None:
    logger.info("🚀 Starting Instagram Follower Tracker v2 (Blastup API)")
    wait_for_internet()
